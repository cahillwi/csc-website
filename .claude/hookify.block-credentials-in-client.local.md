...
conditions:
  - field: file_path
    ...
    pattern: src/app/.*\.(ts|tsx)$|src/components/.*\.(ts|tsx)$
  - field: new_text
    operator: regex_match
    pattern: service_role|SERVICE_ROLE|createServiceRoleClient|ADMIN_KEY|MASTER_KEY|SECRET_KEY.*=.*['"]|SUPABASE_SERVICE_ROLE_KEY|NEXT_PUBLIC_SUPABASE_SERVICE
---
...
**Fix:** Use the standard client for browser code. Move privileged operations to API routes or server actions.
